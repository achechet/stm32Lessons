...
#define USART1_RDR  (*(volatile unsigned long *)0x40013824) 		// Data transmit register

#define NVIC_ISER1  (*(volatile unsigned long *)0xE000E104) 		// Interrupt set enable register

//----------------------------------------------------------------------------------------------------------
// Прототипы функций
void _delay(uint32_t);	// Задержка
uint8_t _strlen(const char* str);		// Возвращает длинну строки

void sendByteUSART1(uint8_t txData);
void sendStringUSART1(const char txData[]);
...
int main(void)
{
	...
	// USART1_TX.PA10 Altenative function 7
	GPIOA_MODER |= (1<<21|0<<20);	// PA9 in alternative mode
	GPIOA_AFRH |= (0<<11|1<<10|1<<9|1<<8); // GPIO alternate function high register AF7 for PA9
	...
	USART_CR1 |= 1<<2;  // Receiver enable

	USART_CR1 |= 1<<5;  // RXNE interrupt enable

	NVIC_ISER1 |= 1<<5;	//Разрешение прерывания 37, USART1
	asm volatile ("CPSIE I");

sendStringUSART1("Start program\n");	// Передаем строку по USART1

    while(1)
    {
    	// sendByteUSART1(0x41);	// Передаем байт
